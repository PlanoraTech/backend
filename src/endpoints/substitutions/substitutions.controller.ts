...
import { Permissions, SpecialPermissions } from '@prisma/client';
...
export class SubstitutionsController {
  ...
  findAll(@Param('institutionId') institutionId: string, @Param('substitutePresentatorId') presentatorId: string) {
  ...
  @Permission([Permissions.READ])
  @SpecialPermission([SpecialPermissions.SUBSTITUTE])
  findOne(@Param('institutionId') institutionId: string, @Param('substitutePresentatorId') presentatorId: string, @Param('id') id: string) {
